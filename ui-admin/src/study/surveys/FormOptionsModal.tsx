...
import { useRuleDesigner } from 'cohort/cohortRule'

/** embeds a form configuration component in a modal */
...
/**
 * Component for configuring a form
 */
export const FormOptions = ({ workingForm, updateWorkingForm }:
{ workingForm: VersionedForm,
  updateWorkingForm: (props: SaveableFormProps) => void}) => {
  const isSurvey = !!(workingForm as Survey).surveyType
  // for now, only Surveys have configurable properties
  let configControls = <></>
  if (isSurvey) {
    const workingSurvey = workingForm as Survey
    const { ruleDesigner } = useRuleDesigner(workingSurvey?.eligibilityRule, rule => updateWorkingForm({
      ...workingForm, eligibilityRule: rule
    }))
    configControls = <div>
